...
int main(int argc, char* argv[]) {    
    ...
    string nucleotides_sequence;
    cin >> nucleotides_sequence;
    
    int nucleotides_sequences = 0;
    
    if (k <= nucleotides_sequence.size()) {
        
        nucleotides_sequences = nucleotides_sequence.size() - k + 1;
        
        if (nucleotides_sequences > DIM_ARRAY_KMERS) {
            
            nucleotides_sequences = DIM_ARRAY_KMERS;
        }
    }
    
    // Obtain the kmers: find the kmers in the input string and put them in an array of Kmers
    for (int i = 0; i < nucleotides_sequences; i++) { 
        
        kmers[i] = nucleotides_sequence.substr(i,k);
    }
    ...
    cout << nucleotides_sequences << endl;
